...
**L’inspecteur de code source permet d’explorer le code source d’une page *web*. Il permet l’identification des balises, des classes, des identifiants, des styles associés aux éléments et ainsi de procéder à une extraction ciblée de données**. En utilisant l’inspecteur de code source, vous accédez à toutes les informations nécessaires pour concevoir un script de collecte automatisée. 
...
Sur la page d'acceuil du site [**ScrapImmob**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html), un ensemble de boutons situé à gauche permet de filtrer les biens par département et par type de biens. **Lorsque l'on clique sur l'un de ces boutons, les biens affichés sur la page sont filtrés en fonction du choix réalisé**. L'URL de la page est alors modifiée de la manière suivante :
...
Notre objectif étant de collecter uniquement des données sur les biens (maisons et appartemments) de la Seine-Maritime, nous allons orienter notre collecte sur les deux URLs suivantes :
...
Ainsi, pour récuperer l'a liste de ces'ensemble des URLs, nous devons donc collecter les valeurs (URL en chaîne de caractère) de l'attribut `href` dans toutes les balises `<a>` de la classe `stretched-link` :
...
```{r scrap_link, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r scraping, echo=TRUE, message=FALSE, warning=FALSE}
...
# Séléction du 2e élément qui stocke les coordonées
code_page_coord <- code_page[2]

```


Avec la fonction `strcapture()`, nous pouvons ensuite extraire les coordonées à l'aide d'une expression régulière qui permet de ne garder que les caractères situés entre crochets `[]`.
...
```{r map_scrap,  echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE}
...
Graphique en barre représentant le nombre de biens en vente par commmune normande :
...
ggplot(scrapimmob, aes(x=prix, y=surf_hab, group=Type)) +
...
# Graphique 
ggplot(data=table, aes(reorder(Group.1, -x), x)) +
 geom_bar(stat="identity", fill = "#4682B4")+ coord_flip() +  
 labs(title = "Le prix des maisons en vente par commune") +
 ylab("Prix médian")+
 xlab("")+
 theme_gray() +
...
La fonction R-base `read.csv()` permet d'exporter le `data.frame` *scrapimmob* qui contient les données collectées.
