...
author: 
 ...
 - name: Alexandre Cebeillac
   affiliation: Université de Rouen, UMR IDEES
...
Au-delà de présenter le *web scraping* avec R, cet article aborde dans un premier temps les connaissances indispensables à la mise en place d'une collecte de données automatisée.
...
# Site, objectifs et prérequis

## Le site web [***ScrapImmob***](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html)

Pour cette introduction à la collecte automatisée de données du *web* avec R, nous proposons une mise en pratique sur un site internet de démonstration crée pour l'occasion : [**ScrapImmob**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html). **Ce site compile 1000 annonces fictives de ventes immobilières de maisons et d'appartements.**  
...
Avant de se lancer dans une collecte automatisée, **il est primordial de bien définir ses objectifs et de déterminer précisément quelles informations on souhaite récupérer. L'exploration du code source du site** (cf. [partie 2.3](#le-code-source-dune-page)) **permet ensuite d'estimer le niveau de difficulté et de choisir une méthode de collecte optimale.**

<div class="alert alert-info">
Pour cette démonstration, **notre objectif est de récupérer une partie de la base de données (un département) des biens mis en vente sur** [**ScrapImmob**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html), incluant l'ensemble des caractéristiques de description. La récupération des coordonnées géographiques affichées dans la carte interactive est également souhaitée.
</div>

**Voici l'ensemble des caractéristiques que nous souhaitons récupérer, pour tous les biens de Seine-Maritime :** 
...
2. Récupération de la liste de toutes les pages (URLs) que l'on souhaite *scraper* ([Partie 3.2.2](#récupérer-les-urls-des-pages-dannonces))
...
**Le scraping d'un site web démarre toujours par la même chose : l'exploration du code source du site ciblé**. En effet, pour automatiser la collecte d'information, il est nécessaire d'inspecter le code source afin de déterminer la localisation précise des différentes informations ciblées. 


Extrait du code source de la page d'acceuil du site web [ScrapImmob](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html) :

<center>

![Source : view-source:https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html](figures/code_source_extarct.png)
...
Dans la majorité des cas, on utilise une balise de fermeture pour indiquer la fin de l’élément. Une balise de fermeture présente un **`/`** avant le nom de la balise :
...
**2. Les sélecteurs d'identifiants** permettent de cibler un élément par son identifiant (**`id`**), précédé du symbole `#`.
...
**3. Les sélecteurs de classes** permettent de cibler tous les éléments d'une même classe. On utilise le `.` pour indiquer qu'il s'agit d'un nom de `class`.
...
**4. Les sélecteurs d'attributs** permettent de cibler des éléments ayant un attribut commun. Exemple : l'attribut `href`, qui renseigne l'URL d'un lien cliquable.   
...
### Exemple pratique - Utilisation de selecteurs CSS pour mettre en forme une page web.
...
3. Ajoutez et/ou modifiez du contenu (balise) dans le code source du fichier page.html. Enregistrez le fichier et ré-affichez (ou rafraichissez avec la touche `F5`) la page HTML dans le navigateur pour découvrir la nouvelle mise en page. 

4. Ajoutez et/ou modifiez du contenu dans le code source du fichier style.css. Enregistrez le fichier et ré-affichez la page HTML (ou rafraichissez-la avec la touche `F5`) dans le navigateur pour apprécier la nouvelle mise en page.
...
Maintenant que vous avez les bases des langages HTML et CSS, vous êtes armé pour consulter le code source du site [**ScrapImmob**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html), comprendre sa structure et localiser les informations que vous souhaitez extraires.


### Affichage brut

Il est très simple d'afficher de manière brut le code source d'une page *web*. Réalisez un clic-droit sur la page ciblée, puis sélectionnez le menu "*(Affichez-le) code source de la page*". Le code source que le navigateur reçoit et met en forme est alors affiché dans un nouvel onglet. Cette fonctionnalité est également accessible avec le raccourci `Ctrl + U`.
...
Vous pouvez déplier le code source pour l'explorer en profondeur. Sur la page d'acceuil de [ScrapImmob](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html), le lien vers les page d'annonces listées sur la page d’accueil semblent stockés dans des balises `<a>` de la classe `stretched-link`.
...
Par exemple, le sélecteur CSS des balises `<a>` de la classe `stretched-link` est `div.col:nth-child(1) > div:nth-child(1) > div:nth-child(3) > a:nth-child(1)`. Ce sélécteur permet de cibler toutes les balise `<a>` de cette classe.
...
<!-- ### Exemple pratique - Utilisation de selecteurs CSS dans la console du navigateur pour isoler des éléments -->

<!-- Pour tester un sélecteur CSS, c'est-à-dire vérifier qu'il isole bien un sous-ensemble des éléments structurés d'une page web, on peut l’exécuter directement dans le navigateur, dans l'onglet `Console` des outils de développement que l'inspecteur ouvre. -->
...
<!-- ::: {style="text-align: center;"} -->
<!-- Avec *Google Chrome* : -->
<!-- ![](figures/browser_console_chrome.png){fig-align="center"} -->
<!-- :::  -->
<!-- :::  -->
<!-- ::: {.column width="2%"} -->
<!-- ::: -->
...
<!-- Dans la console, on peut tester un sélecteur CSS en utilisant une syntaxe spécifique : `$('SelecteurCSS')`. -->

<!-- Par exemple, pour sélectionner tous les liens de cette page web, on peut exécuter la requête `$('a')` (un lien HTML correspondant à une balise `<a>...</a>`) : -->

<!-- ![](figures/browser_console_firefox_selectA.png){fig-align="center"} -->

<!-- Cela renvoie une liste d'objets HTML que l'on peut explorer interactivement (déplier les flèches), et permet ainsi de tester le bon fonctionnement d'une requête via sélecteurs CSS, par exemple avant de l'automatiser dans un script de *web scraping*. -->
<!-- De plus, chacun des éléments isolés peut être localisé dans l'inspecteur (Firefox : clic sur l'icône ![](figures/browser_console_firefox_iconLocate.png) ; Chrome : clic sur le nom de l'élément), et on peut donc contrôler ces résultats. -->

<!-- #### Quelques exercices   -->

<!-- 1. Combien y-a-t-il de liens sur cette page web ?   -->

<!-- 2. Combien y-a-t-il d'éléments en gras sur cette page (balise `<strong>...</strong>`) ? Parmi ces derniers, combien sont également en italique (balise `<em>...</em>`) ?   -->

<!-- 3. En utilisant l'inspecteur HTML et la console, trouvez combien cette page web contient d'extraits de code-source.   -->
...
Comme pour toutes les opérations et manipulations de données, plusieurs solutions existent pour le *web scraping* avec le langage R. Si plusieurs fonctions R-base peuvent être utilisées, il existe surtout deux packages de référence en matière de collecte automatisée :
...
Le package [**`rvest`**](https://rvest.tidyverse.org/) permet de naviguer à travers la structure HTML des pages *web*, d'extraire du texte, des tableaux, des images et d'autres éléments, et de les convertir en données structurées directement exploitables dans R. 
...
Pour cela, nous pouvons utiliser la fonction **`minimal_html()`** qui permet de créer un contenu HTML (XML) requêtable à partir d'un contenu HTML fourni dans une chaîne de caractères.
...
A partir de ces deux URLs, nous allons pouvoir récupérer la liste des URLs de toutes les pages des annonces affichées. Cela nous permettra dans un second temps, de collecter l'ensemble des informations de description de chaque bien en vente. 
...
Ainsi, pour récupérer la liste de ces ensembles d'URLs, nous devons collecter les valeurs (URL en chaîne de caractères) de l'attribut `href` dans toutes les balises `<a>` de la classe `stretched-link` :
...
Cela permet de récupérer les URLs des annonces affichées sur la première page. Pour collecter l'ensemble des biens ciblés, nous devons comprendre comment fonctionne le système d'URL qui permet d'afficher toutes les annonces de biens en vente en Seine-Maritime.
...
Une autre solution aurait été de simuler le clic d'un utilisateur sur le bouton "page suivante". Cela aurait permis de  collecter des données sans avoir à lister les URLs des pages ciblées, mais le script de collecte aurait été plus complexe.</b></div>
...
A partir de cette liste, nous allons pouvoir collecter l'ensemble des URLs présentes sur ces pages, qui renvoient vers des pages d'annonces complètes. Pour cela, nous créons la boucle suivante : 
...
**Les URLs récupérées sont relatives**, c'est-à-dire qu'elles n'incluent que le nom d'une page spécifique. Le chemin courant n'est pas indiqué. Nous pouvons le rajouter avec la fonction `url_absolute()` de la manière suivante.
...
**Les informations que l'on souhaite récupérer dans les balises `<li>` sont affichées en gras**. Cette caractéristique s'avère très précieuse pour optimiser la collecte. 
...
À partir de l'exploration du code source, nous pouvons essayer d'extraire chaque élément séparément.
...
Nous pouvons démarrer la collecte automatisée des informations sur l'ensemble des page ciblées. Nous commençons par créer un tableau contenant les URLs ciblés dans la première colonne puis plusieurs colonne vides qui nous permettront de stocker l'ensemble des données collectées.
...
Nous pouvons désormais construire une boucle qui collecte toutes ses données et complète le tableau précédemment créé.
...
Dans la liste de puce (`<li>`) de la classe `list-group-item` se trouvent plusieurs informations sur les biens. Cela permet de collecter plusieurs informations d'un coup. Cependant, certaines informations ne sont pas toujours renseignées. C'est le cas de "la surface de jardin". Pour cette raison, **nous utilisons un test `if` qui permet d'adapter la collecte en fonction du nombre d'éléments récupérés**.</div> 
...
Cette boucle a permis de compléter le `data.frame` *scrapimmob* de la manière suivante :
...
Il est possible de récupérer les coordonnées géographiques de la localisation des biens en vente indiquées sur la carte interactive, qui sont stockées dans une balise `<script>` dans le code source des pages :
...
Les coordonnées sont stockées dans la seconde balise `<script>`, au sein d'une chaîne de caractère qui englobe beaucoup d'information. Il est donc nécessaire de réaliser quelques traitements afin d'extraire uniquement les coordonnées.
...
<style>
div.notice { background-color:#ECFFDC; border-radius: 5px; padding: 20px;}
</style>
<div class="alert alert-danger">
**Les expressions régulières** (ou <strong>reg</strong>ular <strong>ex</strong>pressions) sont des formules informatiques standardisées, relativement universelles, qui permettent de décrire un *motif* à rechercher dans une chaîne de caractères.
Leur syntaxe est assez obscure, mais extrêmement puissante.
...
Par exemple, l'expression régulière `(0|(\\+33)|(0033))[1-9][0-9]{8}` permet de valider un numéro de téléphone français.</div>
...
La réponse est une unique chaîne de caractères, que l'on peut donc découper avec la fonction `strsplit` là où l'on détecte une virgule suivie d'un espace.
...
1) Création d'un vecteur contenant toutes les URLs absolues vers les photos.
...
Il ne reste plus qu'à utiliser la fonction `wordcloud` du package du même nom pour construire un nuage de mots à partir de ce corpus.
...
Le package `leaflet` (*binding* de la librairie javascript du même nom) nous permet de construire de cartes interactives, et nous allons ici mobiliser la variable `scrapimmob_sf` qui correspond à la conversion en objet spatial vectoriel (sf) de la base de données.
...
Pour autant, la création d'un site dédié comme ici permet certes une maîtrise totale du contenu, mais vise surtout à ne pas lancer de processus de *web scraping* sur un site pour lequel nous n'aurions pas les droits.
Le *web scraping* constitue ainsi une procédure assez agressive pour un site, le principe étant qu'on lui envoi en peu de temps de très multiples requêtes, ce qui engendre un coût certain pour le site visé.
De plus, l'extraction par *web scraping* de l'ensemble du contenu d'un site peut être condamné, comme dans l'exemple du site leboncoin sus-cité qui avait obtenu réparation légale après avoir été scrapé de manière systématique par un concurrent.
...
1. En premier lieu, avant de se lancer dans du *web scraping*, il est indispensable de mener des recherches approfondies pour voir si les données que l'on cherche à collecter ne sont pas déjà disponibles, sous une forme ou une autre, de manière publique ou non. Cela peut prendre la forme de fichiers complets disponibles en open-data, de "*dumps*" de données mis à disposition par les plateformes (*challenges* Kaggle, *datasets* HuggingFace, etc.), ou encore d'interfaces dédiées à l'extraction des données mises à disposition par les sites (les ***API***).

2. Quand vraiment les données ne sont pas disponibles, et elles le sont souvent, alors on peut chercher à contacter les sociétés ou institutions derrière les sites visés, via les contacts listés en fin de page, pour demander des extractions de données.

3. Quand rien de cela n'est concluant, alors seulement on peut se lancer dans le *web scraping* d'un site, en respectant toutefois un principe important de parcimonie : chaque requête envoyée à un site web (`read_html` dans le présent exemple) simule ainsi la requête d'un utilisateur désireux d'accéder au site, et en les démultipliant, on peut engendrer un trafic important vers le site, ce qui a des répercussions financières voire matérielles pour lui.


## Scraping parcimonieux appliqué à cet exemple
...
Les coordonnées auraient pu être récupérées en même temps que le reste du contenu de chaque annonce par exemple.
...
Dans cet exemple, avec une faible perte de qualité de données spatiales (passage de l'adresse à la commune), on passe donc de l'émission de près de 1000 requêtes (sans considérer les photos) à seulement une vingtaine.
