...
INTRO -> HTML = Sous langage XML (Introduire XML - > langage de balisage (utilisé dans word))
...
2. Ouvrez les fichiers page.hmtl et style.CSS avec un editeur de fichier texte (ex : bloc-note, Rstudio, Notepad++...).
...
Définition du Xpath ! langage de requete xml pour naviguer dans un contenu XML (SQL de XML)
...
<b>La première étape du webscraping consiste ainsi à analyser le code source de la page et du site web ciblé, afin de trouver le chemin ou l'emplacement exacte des informations que l'on souhaite récupérer. Plus un site présente une structure HTML optimisée et correctement structuré, plus la collecte automatisé de données sera facilitée.</b>
</div>
...
Micro texte + GIF onglet réseau pour montrer ce qui se passe lorsque l'on charge un opage 
...
Pour cette introduction, notre objectif est de récupérer une partie de la base de données des (Uniquement un département, maisons et appartement) biens mis en vente sur [**ScrapImmob**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html), incluant l'ensemble des caractèristiques décrivant les biens. La récupération des coordonnées géographiques affichées dans la carte intéractive est également souhaitée.



Pour répondre a cet objectif, le processus de collecte va se dérouler en plusieurs étapes :

1. Comprendre la structure de la page d'acceuil/central / Comprendre le système d'URL du site afin de pouvoir récupérer toutes les pages ciblés. / comprendre le système de catégorie , page d'annonce par catégorie = Cmmt le site marche = page d'acceuil filtrée

2. Comprendre comment récupérer les URL des annonces

3. Récuperer la liste des URLs à attaquer

4. Comprendre la structure des pages d'annonces / Ou sont stocké les données/ caractéristiques des biens

5. A partir de la liste des URL de toutes les pages à scraper, collecter l'ensemble des caractèristiques de chaque bien.
...
Comme pour toutes les opérations et manipulations de données, plusieurs solutions existent pour le scraping avec le langage R. Si plusieurs fonctions R-base peuvent être utilisées, il existe surtout de package R de référence en matière de collecte automatisée :

- [`rvest`](https://rvest.tidyverse.org/). Developpé par Hadley Wickham (Rstudio), ce package facilite le scraping en utilisant les sélecteurs CSS et XPath pour extraire les données formatées en HTML ou en XML. Ce package s'insprire des bibliothèques python déjà existantes : [Beautiful Soup](https://www.crummy.com/software/BeautifulSoup/) et [RoboBrowser](https://robobrowser.readthedocs.io/en/latest/readme.html). `rvest` fait partie de l'écosystème `tidyverse`, les fonctions peuvent s'utiliser avec le(s) pipe(s). Ce package est simple d'utilisation mais également limité en matière de fonctionnalité. Il ne permet pas de simuler des actions utilisateur (clic, scroll...). On l'utilisera essentiellement pour l'extraction de données de sites web statiques bien structurée.

- [`Rselenium`](https://cran.r-project.org/web/packages/RSelenium/index.html). Ce package est ce qu l'on appelle un *bindings* ("package de liason" ??) qui permet d'utiliser les fonctionnalités de `Selenium 2.0 WebDriver`. Ce framework web permet d'automatiser les tests d’applications Web en simulant des utilisateurs. `Selenium 2.0 WebDriver` peut aussi être utilisé pour la collecte automatisée de données sur des sites web dynamiques, nécessitant une interaction utilisateur (exécution de code javascript généré par clic, survol, scroll...)
...
Commençons à explorer les principales fonctions du package <code>rvest</code> à partir d'un extrait de code html du site [**ScrapImmob**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html) : 
...
### Scraping de la première page du site


 !! D'abord détécter l'URL de la page de sous séléction dans le code source !!

[**ScrapImmob**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html)
