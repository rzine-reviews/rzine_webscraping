...
nocite: |
  @*
link-citations: true
github: "rzine-reviews/rzine_webscraping"
# gitlab: "gitlab.huma-num.fr/webscraping/rzine_webscraping"
doi: "10.48645/apqy-q532"
licence: "by-sa"
params:
...
Il peut s'agir de texte (faisant parfois référence à des entités géographiques telles que des longitudes ou des latitudes), de tableaux, d'images ou encore de vidéos.
...
En sciences humaines et sociales, le *web scraping* peut être utilisé pour collecter et structurer efficacement des données sur des sites *web*, forums, blogs, des réseaux sociaux ou autres sources en ligne.
Par exemple, si des centaines de tableaux de données sont disponibles sur un site *web* dont l'analyse permettrait d'étayer différentes hypothèses de recherches, il paraît préférable d'en automatiser la collecte plutôt que de les copier/coller manuellement.
...
L'utilisation de R pour le *web scraping* permet d'automatiser ce processus et d'analyser directement les données collectées.

Avant de se lancer dans du *web scraping*, il convient d'avoir une vision claire sur les utilisations des données que l'on souhaite récupérer.
Ce n'est pas parce qu'on peut télécharger toutes les données d'un site *web* que l'on doit le faire.
En plus de générer inutilement du trafic réseau qui peut perturber le fonctionnement du site, il faut se poser des questions simples : quelles données me semblent nécessaires pour répondre à ma question de recherche ?
Ai-je vraiment besoin de toutes ces données ?
Aurais-je les capacités et l'opportunité de tout traiter ?
N'y a-t-il pas un autre moyen plus simple et moins *gris* de les obtenir ?

Le *web scraping* permet de récupérer des informations spécifiques d'une page *web*, notamment en analysant sa structure HTML et en extrayant uniquement les éléments pertinents.
Cette pratique demande quelques connaissances techniques en matière de *web*, de langage de balisage HTML et de langage CSS, qui permettent respectivement de structurer une page *web* et de définir l'apparence et la mise en forme des différents éléments du document.

Le *web scraping* regroupe différentes techniques plus ou moins complexes, selon la structure du site cible.
Ce dernier peut être statique, c'est à dire que le contenu de la page ne varie pas entre les visites, ou *a contrario* dynamique.
...
**Ce site compile 1 000 annonces fictives de ventes immobilières de maisons et d'appartements.**
...
::: {.alert .alert-danger}
<b>L'intégralité des biens présentés, ainsi que les images associées ont été générés par différents modèles d'intelligence artificielle.
Toutes les informations affichées sont donc complètement fictives.
Toute ressemblance avec une annonce existante ou ayant existé serait purement fortuite et ne pourrait être que le fruit d'une pure coïncidence.</b>
:::
...
-   un **titre**,\
-   une **description**,\
-   des **caractéristiques techniques** (surface, prix, nombre de pièces, etc.),\
...
Avant de se lancer dans une collecte automatisée, **il est primordial de bien définir ses objectifs et de déterminer précisément quelles informations nous souhaitons récupérer.** **L'exploration du code source du site** (cf. [partie 2.3](#explorer-le-code-source)) **permet ensuite d'estimer le niveau de difficulté et de choisir une méthode de collecte optimale.**

::: {.alert .alert-info}
Pour cette démonstration, notre objectif est de récupérer la liste des biens mis en vente (maisons et appartements) en Seine-Maritime sur le site [**ScrapImmob**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html){target="_blank"}, incluant l'ensemble des caractéristiques de description.
...
4.  Collecte de l'ensemble des caractéristiques de chaque bien à partir de la liste des URLs récupérées ([Partie 3.3.2](#construction-dun-tableau-de-collecte))

5.  Nettoyage et traitement des données collectées ([Partie 3.4](#nettoyage-des-données))

6.  Exploration du code source pour récupérer les coordonnées géographiques (carte interactive) ([Partie 3.5](#collecte-de-coordonnées))
...
Ce dernier prend la forme d'un fichier texte qui contient l'ensemble des instructions codées dans un langage informatique donné et qui permettent notamment d'afficher le site *web* tel que vous le voyez.
Selon la philosophie de conception du site *web* (statique ou dynamique), les informations qui apparaissent à l'écran peuvent se retrouver directement dans le texte du code source, à des emplacements définis par le/les concepteurs du site.
Ainsi, pour automatiser la collecte d'informations, il est nécessaire d'inspecter le code source pour comprendre la structure et l'organisation de la page *web* afin de déterminer la localisation précise des différentes informations ciblées que l'on cherchera à extraire.
...
**Il est indispensable d'avoir certaines connaissances liées à la structure d'une page *web* et une compréhension des principes techniques sous-jacents.** **Vous devez ainsi acquérir une connaissance minimum du langage HTML (balises) et une familiarité avec le CSS (sélecteurs) pour identifier, cibler et extraire les éléments d'une page** ***web***.
...
Le langage HTML (*Hyper Text Markup Language*) est un langage informatique qui permet de décrire le contenu d'une page web.
Comme le XML (*eXtensible Markup Language*), son langage cousin, le HTML repose sur l'utilisation de balises (définies au sein de chevrons `<>`).

Le XML peut être utilisé dans une variété de domaines, notamment pour l'échange de données entre applications.
Par exemple, voici un document XML simple contenant des informations sur un livre :
...
Le langage HTML partage de nombreuses caractéristiques avec le XML[^1], mais il est spécifiquement conçu pour structurer et organiser le contenu des pages *web*.

[^1]: *Via* un héritage commun depuis [le langage SGML](https://fr.wikipedia.org/wiki/Standard_Generalized_Markup_Language)

Les normes HTML (HTML5 par exemple) définissent un [ensemble important mais circonscrit de balises](https://developer.mozilla.org/fr/docs/Web/HTML/Element){target="_blank"} (des *tags*) pour définir la structure et le contenu d'une page *web*, ce qui permet aux navigateurs *web* de l'afficher correctement pour les utilisateurs.
...
Cependant, il existe un ensemble de balises HTML standard définies dans les spécifications du W3C (*World Wide Web Consortium*[^2]).
...
[^2]: « Le ***World Wide Web Consortium***, abrégé par le sigle ***W3C***, est un organisme de standardisation à but non lucratif, fondé en octobre 1994 chargé de promouvoir la compatibilité des technologies du *World Wide Web* telles que HTML5, HTML, XHTML, XML, RDF, SPARQL, CSS, XSL, PNG, SVG, MathML et SOAP. 
    » [Wikipedia, 10/09/2024](https://fr.wikipedia.org/wiki/World_Wide_Web_Consortium)
...
-   **`<!DOCTYPE html>`** : En début du document, indique au navigateur qu'il s'agit d'une page HTML5[^3].
...
[^3]: « **HTML5** (*HyperText Markup Language 5*) est la dernière révision majeure du HTML (format de données conçu pour représenter les pages *web*), finalisée le 28 octobre 2014.
    HTML5 spécifie deux syntaxes d'un modèle abstrait défini en termes de DOM : HTML5 et XHTML5. 
    », [Wikipedia, 10/09/2024](https://fr.wikipedia.org/wiki/HTML5)
...
Le `body` peut englober une variété de balises prédéfinies pour structurer et ajouter différents types de contenu dans la page *web*.
...
Dans l'exemple de code source ci-dessus, il est ainsi possible de récupérer toutes les adresses web (ou URLs pour *Uniform Resource Locator*) des articles, en ciblant le contenu de tous les attributs **`href`** des balises **`<a>`** situés dans la **`<div>`** ayant pour identifiant "*Liste_ref*".

\

## Le langage CSS {#le-langage-css}
...
Cette dernière méthode est à privilégier car la séparation du contenu et de la mise en forme facilite la mise à jour des styles.
...
Dans les trois cas présentés ci dessus, le rendu graphique des titres de niveau 1 dans un navigateur *web* sera :
...
::: {.alert .alert-danger}
L'utilisation du symbole `+` permet de cibler uniquement la première balise `h2` de la balise ayant pour identifiant `liste_ref`.
:::
:::
:::
...
<center>[<img src="figures/zip.png" width="30"/> **exo_HTML.zip**](https://rzine.fr/docs/20250711_webscraping/exo_HTML.zip)</center>
...
2.  Ouvrez les deux fichiers avec un éditeur de fichier texte (ex : bloc-note, RStudio, Notepad++, etc.).
...
3.  Modifiez le code source du fichier *page.html*, en ajoutant un titre de niveau 2 et un paragraphe dans le `body`.
...
4.  Modifiez du contenu dans le code source du fichier *style.css*.
    Ajoutez une couleur pour les titres de niveau 2 et paramétrez les paragraphes en italique.
...
![](figures/correction_exo_html.png)

<center>[<img src="figures/zip.png" width="30"/> **correction_exo_HTML.zip**](https://rzine.fr/docs/20250711_webscraping/correction_exo_HTML.zip)</center>

<br>

\

## Explorer le code source {#explorer-le-code-source}
...
Réalisez un clic-droit sur la page ciblée, puis sélectionnez l'option "*(Affichez le) code source de la page*".
...
Avec *Google Chrome* : ![](figures/view_source_chrome.png){fig-align="center"}
:::
:::
...
::: {.column width="49%"}
::: {style="text-align: center;"}
Avec *Mozilla Firefox* : ![](figures/view_source_firefox.png)
:::
:::
:::
...
En l'utilisant, vous accédez à toutes les informations nécessaires pour concevoir un script de collecte automatisée.

Pour l'utiliser, cliquez-droit n'importe où sur la page *web* ouverte dans un navigateur, puis cliquez sur "Inspecter" :
...
XPath est ainsi utilisé pour sélectionner des éléments spécifiques d'une page *web*.
:::
...
En pratique, si le navigateur indique bien que le sélecteur correspondant à cette dernière requête est `div.col:nth-child(1) > div:nth-child(1) > div:nth-child(3) > a:nth-child(1)`, on notera que la récupération du premier élément de `a.stretched-link` (par exemple en faisant `$$('a.stretched-link')[0]` dans la console, cf.
*infra*) est plus lisible.
:::
...
::: {style="text-align: center;"}
Avec *Mozilla Firefox* : ![](figures/browser_console_firefox.png){fig-align="center"}
:::

Dans la console, on peut tester un sélecteur CSS en utilisant la syntaxe spécifique suivante : `$$('sélecteur')`\
Par exemple, pour sélectionner tous les liens (balises `<a>...</a>`) de cette page *web*, on peut exécuter la requête `$$('a')` dans la console de l'inspecteur.
Avec *Mozilla Firefox* :
...
Chacun des éléments isolés peut être localisé dans l'inspecteur (clic sur l'icône ![](figures/browser_console_firefox_iconLocate.png) avec *Firefox*, puis clic sur le nom de l'élément avec le navigateur *Chrome*).
...
::: {.alert .alert-info}
Nous proposons ici une courte série d'exercices corrigés (cliquer sur `Réponse`) qui doivent permettre de mener une première exploration du code source du site exemple
:::
...
<br />
Pour connaître le nombre de liens de la page citée, on peut mener une recherche des éléments <code>&lt;a&gt;</code> de la page, en tappant la requête suivante dans la console : <code>$$('a')</code>
...
**2.** Combien y-a-t-il de liens (balises `<a>...</a>`) de la classe `.stretched-link` ?
Vous pouvez utiliser la syntaxe suivante : `$$('element.class')`
...
On peut ici cumuler des sélecteurs, en ajoutant la classe <code>.stretched-link</code> au sélecteur des liens <code>a</code> : <code>$$('a.stretched-link')</code>
...
Même logique que pour la recherche d'éléments <code>&lt;a&gt;</code>, mais cette fois-ci pour les éléments <code>&lt;strong&gt;</code> : <code>$$('strong')</code> 
...
**4.** Combien de balises `<script>...</script>` permettent d'intégrer ou de faire référence à un script javascript exécutable ?
Dans quelle balise `<script>` sont stockées les coordonnées géographiques du marqueur affiché sur la carte interactive des [pages d'annonces](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/annonces/IDF_0049.html){target="_blank"} ?
...
Encore la même logique que pour la recherche d'éléments <code>&lt;a&gt;</code>, mais cette fois-ci pour les éléments <code>&lt;script&gt;</code> : <code>$$('script')</code> 
...
## Les packages de *scraping* {#les-packages-de-scraping}

Comme pour toutes les opérations et manipulations de données, plusieurs solutions existent pour le *web scraping* avec R.
Si plusieurs fonctions R-base[^4] peuvent être utilisées, il existe surtout deux packages de référence en matière de collecte automatisée :

[^4]: L'appellation R-base désigne l'ensemble des fonctions natives de R, c'est-à-dire installées d'office avec le logiciel, sans ajout de *packages* supplémentaires.
    On oppose fréquemment les fonctions R-base et les fonctions du *tidyverse*, qui utilisent une approche syntaxique assez différente.
...
-   [**`rvest`**](https://rvest.tidyverse.org/){target="_blank"} : développé par Hadley Wickham (Posit), ce package facilite le *scraping* en utilisant les sélecteurs CSS et XPath pour extraire les données formatées en HTML ou en XML.
    ...
    **`rvest`** fait partie de l'écosystème **`tidyverse`**, les fonctions peuvent s'utiliser avec le(s) *pipe(s)*.
    Il est simple d'utilisation mais limité en matière de fonctionnalité.
...
-   [**`RSelenium`**](https://cran.r-project.org/web/packages/RSelenium/index.html){target="_blank"} : ce package est ce que l'on appelle un *binding* de langage ("liaison") qui permet d'utiliser, depuis R, les fonctionnalités de l'outil **`Selenium 2.0 WebDriver`**.
    ...
    **`Selenium 2.0 WebDriver`** peut aussi être utilisé pour la collecte automatisée de données sur des sites *web* dynamiques, nécessitant une interaction utilisateur (exécution de code javascript généré par clic, survol, scroll, etc.).
    Il s'agit concrètement d'un outil qui simule la navigation sur un site *web* plutôt qu'un outil comme `rvest` qui en extrait simplement le contenu.
...
Pour cela, nous pouvons utiliser la fonction [**`minimal_html()`**](https://rvest.tidyverse.org/reference/minimal_html.html){target="_blank"} qui permet de créer un contenu HTML (XML) requêtable à partir de code HTML fourni dans une chaîne de caractères.
...
Les différentes fonctions du package `rvest` renvoient le plus souvent des objets `xml_` ou `html_` (`xml_document` et `xml_node` dans cet exemple, `xml_nodeset` dans le suivant), qui sont des pointeurs vers le contenu XML/HTML interpreté, et peuvent donc être eux-mêmes interrogés par d'autres fonctions de `rvest`.
L'équivalent d'une telle structure en objets R classiques serait une structure emboîtée de listes contenant des listes, contenant elles-mêmes des listes (et ainsi de suite).

-   Les fonctions [**`html_element()`**](https://rvest.tidyverse.org/reference/html_element.html){target="_blank"} ou `html_elements()` permettent respectivement la récupération du 1er ou de tous les éléments d'un document HTML, en précisant un sélecteur CSS ou un Xpath. Par exemple, `html_elements("p")` appliqué au code source d'une page permet de récupérer tous les paragraphes :
...
-   La fonction [**`html_attr()`**](https://rvest.tidyverse.org/reference/html_attr.html){target="_blank"} permet l'extraction des valeurs d'attributs.
...
-   Les fonctions [**`html_text()`**](https://rvest.tidyverse.org/reference/html_text.html){target="_blank"} ou `html_text2()` permettent d'extraire uniquement le texte d'un élément.
...
::: {.alert .alert-danger}
Les fonctions du package `rvest` sont conçues pour être utilisées en *pipe*.Elles fonctionnent aussi bien avec le *pipe* natif du R-base (`|>`), qu'avec celui du package `magrittr` (`%>%`), utilisé par le `tidyverse`
:::

\

## Le système d'URL {#le-système-durl}
...
**Lorsque l'on clique sur l'un de ces boutons, les biens affichés sur la page sont filtrés en fonction du choix réalisé.** **L'URL de la page est alors modifiée** de la manière suivante :

**URL principale** :\
[[https://analytics.huma-num.fr/Robin.Cura/**ScrapImmob/index.html**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html){target="_blank"}]{style="font-size: 90%;"}

**URL du filtre "Essonne (91) - Appartement"** :\
[[https://analytics.huma-num.fr/Robin.Cura/**ScrapImmob/91_appartement/index.html**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/91_appartement/index.html){target="_blank"}]{style="font-size: 90%;"}

**URL du filtre "Val-d'Oise (95) - Maison"** :\
[[https://analytics.huma-num.fr/Robin.Cura/**ScrapImmob/95_maison/index.html**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/95_maison/index.html){target="_blank"}]{style="font-size: 90%;"}
:::
:::
...
-   [[https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/**76_maison/index.html**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/76_maison/index.html){target="_blank"}]{style="font-size: 90%;"}\
-   [[https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/**76_appartement/index.html**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/76_appartement/index.html){target="_blank"}]{style="font-size: 90%;"}

Nous commençons par récupérer le code source de ces deux pages avec la fonction [**`read_html()`**](https://rvest.tidyverse.org/reference/read_html.html){target="_blank"}.
...
### Localiser les URLs des pages d'annonces {#localiser-les-urls-des-pages-dannonces}
...
### Récupérer toutes les URLs {#récupérer-toutes-les-urls}
...
::: {style="display: flex;"}
::: {.column width="47%"}
<center><b>[**../76_appartement/index.html**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/76_appartement/index.html){target="_blank"}</b></center>
...
::: {.column width="47%"}
<center><b>[**../76_maison/index.html**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/76_maison/index.html){target="_blank"}</b></center>
...
::: {.alert .alert-danger}
Une autre solution aurait été de simuler le clic d'un utilisateur sur le bouton "page suivante" (par exemple en utilisant le package RSelenium [mentionné précédemment](#les-packages-de-scraping)).
Cela aurait permis de collecter des données sans avoir à lister les URLs des pages ciblées, mais cela aurait nécessité un script plus complexe alors que le système d'URLs s'avère extrêmement simple.
:::

\

### Collecter toutes les URLs {#collecter-toutes-les-urls}

À partir de cette liste d'URLs, nous allons pouvoir collecter l'ensemble des liens qui renvoient vers des pages d'annonces complètes.
Pour cela, nous créons la boucle suivante :
...
Nombre total de liens de page d'annonce récupéré :
...
Nous pouvons le rajouter avec la fonction [`url_absolute()`](https://xml2.r-lib.org/reference/url_absolute.html) de la manière suivante.
...
## Collecte principale {#collecte-principale}
...
::: {.alert .alert-info}
Les informations que l'on souhaite récupérer dans les balises `<li>` sont affichées en gras (balise `<strong>`). Cette caractéristique s'avère très précieuse pour optimiser la collecte.
:::
...
::: {.alert .alert-info}
La récupération des coordonnées du marqueur affiché dans la carte interactive est légèrement plus complexe. Leur collecte automatisée est expliquée dans la [partie 3.5](#collecte-de-coordonnées).
:::

\

### Construction d'un tableau de collecte {#construction-dun-tableau-de-collecte}
...
Nous pouvons maintenant construire une boucle qui collecte toutes les données désirées et remplit le tableau précédemment créé.
...
# Récupération Type, Nb de chambre, de salle de bain, surfaces, prix, commune et département
...
::: {.alert .alert-danger}
Dans la liste à puces (balise `<li>` de la classe `list-group-item`) se trouvent plusieurs informations sur les biens.
...
## Nettoyage des données {#nettoyage-des-données .tabset}

**En *web scraping*, le plus compliqué n'est pas forcément la collecte des données.** **Un prétraitement des données collectées est souvent nécessaire pour obtenir une base de données propre et optimisée.**[^5]

[^5]: Nous privilégions ici un nettoyage écrit en R, faisant appel notamment à des fonctions parfois avancées de manipulation de chaînes de caractères, mais il existe aussi des outils graphiques plus conviviaux permettant de mener cette phase importante, [OpenRefine](https://openrefine.org/) par exemple.
...
Nous procédons donc à des traitements pour nettoyer les informations collectées, en mobilisant notamment la fonction [`gsub()`](https://rdrr.io/r/base/grep.html) qui permet de rechercher et remplacer un motif (*pattern*) dans un texte.
...
# Conversion en numerique des colonnes ciblées
# via la fonction as.numeric()
# appliquée aux colonnes du tableau via lapply()
...
## Collecte de coordonnées {#collecte-de-coordonnées}

La collecte des coordonnées géographiques des biens en vente indiquées sur la carte interactive est un peu plus difficile.
...
Les coordonnées sont stockées dans la deuxième balise `<script>`, au sein d'une chaîne de caractères qui englobe beaucoup d'informations.
Il est donc nécessaire de réaliser quelques traitements afin d'extraire uniquement les coordonnées.
Nous commençons par ne garder que les informations de la deuxième balise `<script>`.

```{r, echo=TRUE, message=FALSE, warning=FALSE, cache=TRUE, eval=TRUE}

# Sélection du 2e élément (balise script) qui contient les coordonnées
code_page_coord <- code_page[2]

```

Puis la fonction [`strcapture()`](https://rdrr.io/r/utils/strcapture.html) nous permet d'extraire les coordonnées à l'aide d'une **expression régulière**[^6] qui ne conserve que les caractères situés entre crochets `[]`.

[^6]: **Les expressions régulières** (ou <strong>reg</strong>ular <strong>ex</strong>pressions, <strong>REGEX</strong>) sont des formules informatiques standardisées, relativement universelles, qui permettent de décrire un *pattern* à rechercher dans une chaîne de caractères.
    Leur syntaxe est difficilement intelligible, mais extrêmement puissante.
    ...
    Par exemple, l'expression régulière `(0|(\\+33)|(0033))[1-9][0-9]{8}` permet de valider un numéro de téléphone français (doit commencer par 0, +33 ou 0033 puis doit être suivi d'un chiffre de 1 à 9, puis de huit chiffres de 0 à 9).
...
À l'aide de la fonction [`strsplit()`](https://rdrr.io/r/base/strsplit.html), on découpe cette chaîne de caractères là où l'on détecte une virgule suivie d'un espace.
Cela génère une liste de deux éléments que l'on peut ensuite dé-lister (fonction [`unlist()`](https://rdrr.io/r/base/unlist.html)) pour obtenir un vecteur textuel contenant les deux coordonnées.
...
# Ajout des coordonnées dans le data.frame scrapimmob
...
# Conversion des coordonnées récupérées en format numerique
...
Notre base de données collectées liste les 419 biens à vendre (maisons et appartements) en Seine-Maritime, caractérisés par plusieurs variables :
...
La fonction native [`write.csv()`](https://rdrr.io/r/utils/write.table.html) permet d'exporter le `data.frame` *scrapimmob* qui contient l'ensemble des données collectées.
...
Le package `sf` pour la manipulation de données géographiques vectorielles permet, avec sa fonction [`st_as_sf()`](https://r-spatial.github.io/stars/reference/st_as_sf.html), de convertir le `data.frame` *scrapimmob* en couche géographique ponctuelle (objet `sf`), à partir des latitudes et des longitudes stockées en colonne.
...
plot(st_geometry(scrapimmob_sf))

```

Notre base de données est désormais une couche géographique vectorielle (objet `sf`, schématiquement un tableau de données avec une colonne décrivant la géométrie).
Nous pouvons l'exporter dans le format de fichier de notre choix, par exemple en format geopackage :
...
## Téléchargement des photos {#téléchargement-des-photos}
...
1)  Création d'un nouveau répertoire pour y stocker l'ensemble des images collectées.
...
3)  En utilisant la fonction [`download.file()`](https://rdrr.io/r/utils/download.file.html), nous pouvons construire une boucle pour enregistrer toutes les photos dans le nouveau répertoire "*pictures*".
...
# Paramétrage de l'affichage des chiffres dans le graphique
options(scipen = 999)

# Sélection des maisons
scrapimmobmaison <- scrapimmob[scrapimmob$Type == "Maison",]

# Regroupement par commune - calcul du prix médian
table <- aggregate(
...
Nous commençons par créer un objet `list` de type corpus (fonctions [`Corpus()`](https://rdrr.io/rforge/tm/man/Corpus.html) et [`VectorSource()`](https://rdrr.io/rforge/tm/man/VectorSource.html) du package `tm`), à partir de la variable `Description`.
...
La fonction [`tm_map()`](https://rdrr.io/rforge/tm/man/tm_map.html) permet ensuite de nettoyer notre corpus textuel.
Après avoir converti tout le texte en minuscules, nous supprimons les *stopwords*, la ponctuation et les chiffres.
...
La fonction [`wordcloud()`](https://rdrr.io/cran/wordcloud/man/wordcloud.html) permet ensuite de construire un nuage de mots.
...
Le package [`leaflet`](https://cran.r-project.org/web/packages/leaflet/){target="_blank"} (*binding* de la librairie javascript du même nom) nous permet de construire des cartes interactives. Nous allons ici mobiliser la variable `scrapimmob_sf` qui correspond à la conversion en objet spatial vectoriel (sf) de la base de données des biens mis en vente.
...
La fonction [`leaflet()`](https://rstudio.github.io/leaflet/reference/leaflet.html) nous permet de cartographier les biens mis en vente, colorés selon le type.
...
<center>Le survol et le clic sur les points affichés permettent de consulter les informations collectées.</center>
...
Le site *web* exemple utilisé dans cet article est certes entièrement fictif, généré par un savant mélange de données *data-driven* et d'appel à des outils d'IA (*Large Language Model* pour les descriptions des annonces, *Text-to-Image Model* pour les "photos"), mais n'en reste pas moins réaliste.
La plupart des sites d'annonces, immobilières ou généralistes, comme [Leboncoin](https://www.leboncoin.fr/){target="_blank"}, adoptent en effet une structure similaire, où des pages de catalogage d'annonces, correspondant à des filtres sur la base de données globale, permettent d'accéder aux vignettes des annonces individuelles, que l'on peut ensuite parcourir individuellement.
...
Le *web scraping* constitue en effet une procédure agressive et un coût certain pour un site, car une multitude de requêtes sont envoyées en peu de temps.

L'extraction par *web scraping* de l'ensemble du contenu d'un site peut être condamnée, comme dans l'exemple du site [Leboncoin](https://www.leboncoin.fr/){target="_blank"} sus-cité qui avait obtenu réparation légale après avoir été scrapé de manière systématique par un concurrent.
...
Cela peut prendre la forme de fichiers complets disponibles en *open data*, de "*dumps*" de données mis à disposition par les plateformes (*challenges* Kaggle, *datasets* HuggingFace, etc.)[^7], ou encore d'interfaces dédiées à l'extraction des données mises à disposition par les sites (les *API*).

[^7]:  Depuis le début des années 2010, dans un contexte de large démocratisation des méthodes de *machine-learning*, plusieurs plateformes sont apparues pour susciter des compétitions publiques de *machine-learning* adossées à des jeux de données usuellement fermés.
    Les entreprises fournissant leurs données à ces plateformes en tirent des méthodes et algorithmes pour résoudre leurs problèmes de classifications et analyses prédictives, et le public peut en contrepartie accéder à ces jeux de données *via* les plateformes de compétition, telle [Kaggle](https://www.kaggle.com/datasets).
    Avec l'augmentation de la visibilité des recherches en IA depuis les années 2020, d'autres plateformes comme [HuggingFace](https://huggingface.co/datasets) ont émergé pour partager des modèles libres, qui donnent de plus accès à des jeux de données importants utilisés pour l'apprentissage ou l'évaluation de ces modèles.

Dans le cas où les données ne semblent pas disponibles, n'hésitez pas à contacter directement les données au producteur.

### *Scraper* avec parcimonie

Si le *scraping* est le seul moyen d'accéder aux données ciblées, vous pouvez vous lancer.
Néanmoins, il est important de respecter un principe de parcimonie : chaque requête envoyée à un site *web* (fonction `read_html()`) simule la requête d'un utilisateur désireux d'accéder au site ; en les démultipliant, on peut engendrer un trafic important vers le site, ce qui peut avoir des répercussions financières voire matérielles.
...
C'est de plusieurs ordres de grandeur supérieur au comportement d'un utilisateur "réel" du site, et cela peut entraîner des erreurs sur le serveur physique qui héberge le site, d'autant plus si une vingtaine de personnes exécutent ces mêmes requêtes simultanément.
...
Pour vous aider à mettre en place des processus de *web scraping* plus complexes, nous vous renvoyons vers un site *web* associée à cette publication : <https://webscraping.gitpages.huma-num.fr/website/>.
...
rref <- bibentry(
   ...
   title = "Premiers pas en web scraping",
   subtitle = "Introduction à la collecte automatique de données du web",
   author = c("Léa Christophe", "Hugues Pecout", "Robin Cura", "Alexandre Cebeillac", "Sébastien Rey-Coyrehourcq"),
   doi = "10.48645/10.48645/apqy-q532",
   url = "https://rzine.fr/bibliography/article_10/",
