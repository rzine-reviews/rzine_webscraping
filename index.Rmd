...
- l'intérieur du HTML (c'est-à-dire le contenu de la balise sélectionnée : l'intérieur du HTML de `<div id="abc" class="foo"><strong>TEXTE</strong></div>` est  ̀<strong>TEXTE</strong>`)
...
Il s'agit donc d'observer les *appels* qu'une page émet lors de son chargement, par exemple pour récuperer des images, des polices, des bibliothèques de mise en page (CSS, javascript, etc.).
...
{{< video figures/Screencast_Inspecteur_Network_crop.webm >}}

Dans cet exemple d'inspection de cette même page, on constate que l'ouverture de la page de cet article déclenche :  
1. L'ouverture de la page à proprement parler (format  ̀html` dans la colonne `Type`)
...
### Récupération des URL ciblées

Maintenant, nous allons nous exercer sur la [première page du site ScrapImmob](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html). Nous récupérons le code de la page à l'aide de la fonction `read_html` : 
