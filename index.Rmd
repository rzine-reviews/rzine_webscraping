...
Pour isoler les caractères situés entre crochet dans la balise `script`, on utilise l'expression régulière "`\\[(.*?)\\]`", qui peut se décomposer ainsi :  
...
La réponse est une unique chaîne de caractères, que l'on peut donc découper avec la fonction `strsplit` là où l'on détecte une virgule suivi d'un espace.
Cela génère une liste de deux éléments, que l'on peut ensuite délister pour obtenir un vecteur textuel contenant les deux coordonnées.
...
```{r, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE, cache = FALSE}
stopwords_wordcloud <- c(stopwords("french"), "m2", "€")
...
Le package `leaflet` (*binding* de la librairie javascript du même nom) nous permet de construire de cartes interactives, et nous allons ici mobiliser la variable `scrapimmob_sf` qui correspond à la conversion en objet spatial vactoriel (sf) de la base de données.
...
0) Ouverture le bon coin thématique : même logique dans ce faux site et dans le site réel, collection d'annonces individuelles : on peut récuperer des résumés de plusieurs annonces (pages résultats) ou les détails de chaque annonce (page annonce)
1) Questions ethiques + exemple le bon coin ethique
