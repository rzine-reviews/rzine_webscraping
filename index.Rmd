...
En sciences humaines et sociales, le *web scraping* peut être utilisé pour collecter des données pertinentes sur des sites *web*, forums, blogs, des réseaux sociaux, ou autres sources en ligne. Par exemple, si des centaines de tableaux de données sont disponibles sur un site web et dont l'analyse permettrait d'étayer différentes hypothèses de recherches, il parait préférable d'en automatiser la collecte plutôt que de les copier/coller manuellement.
...
Mais avant de se lancer dans du *web scraping*, il convient d'avoir une vision claire sur les utilisations futures des données que l'on souhaite récupérer. Ce n'est pas parce qu'on peut télécharger toutes les données d'un site web que l'on doit le faire. En plus de générer inutilement du traffic réseau qui peut pertuber le fonctionnement du site, il faut se poser des questions simples : quelles données me semblent nécessaires pour répondre à ma question de recherche ? est-ce que j'ai vraiment besoin de toutes ces données ? Aurais-je les capacités et l'opportunité de tout traiter ? et est-ce qu'il n'y a pas un autre moyen plus simple et moins *gris* de les obtenir ?
...
Ce dernier prend la forme d'un fichier texte qui contient l'ensemble des instructions codées dans un langage informatique donné et qui permettent notamment d'afficher le site web tel que vous le voyez actuellement. Selon la philosophie de conception du site web (statique ou dynamique), les informations qui apparaissent à l'écran peuvent se retrouver directement dans le texte du code source, à des emplacements définis par le/les concepteur du site.
Ainsi, pour automatiser la collecte d'informations, il est nécessaire d'inspecter le code source pour comprendre la structure et l'organisation de la page web afin de déterminer la localisation précise des différentes informations ciblées que l'on cherchera a extraire.
