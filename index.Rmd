...
### Collecte des URLs des pages d'annonce
...
Losrque l'on observe le comportement de l'URL à chauqe chargement d'une nouvelle page, il est facile de comprendre le système d'URLs utilisé par [**ScrapImmob**](https://analytics.huma-num.fr/Robin.Cura/ScrapImmob/index.html), exemple :
...
# Remplacement de la première page qui s'appelle index
list_url_appart[1] <- gsub(x = list_url_appart, pattern =  "page_1.html", replacement = "index.html")


# Récupération des URLs pour les biens de type "maison" (76) 
list_url_maison <- paste0(url_maison, "page_", 1:nbpage_maison,".html")
list_url_maison[1] <- gsub(x = list_url_maison, pattern =  "page_1.html", replacement = "index.html")
...
L'utilisation du package `rselenium` nous aurait permis de simuler un clic d'utilisateur sur le bouton "*page suivante*", et ainsi de collecter les informations de l'ensemble des pages sans se soucier du système d'URL. Cette étape n'aurait pas été nécéssaire.</b></div>

\


### Scraping des URLs de tous les biens ciblés
...
Les URLs récupérées semblent relatives (uniquement la fin), nous allons donc reconstituer les URLs à l'aide de la fonction `gsub()` :
...
- Description : Balise `<p>` de classe `card-text text-start`

Les autres informations sont stockées dans une même  balise `<ul>` (liste) de classe `list-group` :

- Type de bien : Balise `<h6>` de classe `card-text text-start`
- Nombre de chambre
- Nb de salle de bain
- Surface habitable
- Prix
