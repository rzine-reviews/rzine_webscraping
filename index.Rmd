...
- **Surface jardin** : balise `<li>` de classe `list-group-item` - **élément facultatif**     
...
Certaines des informations collectées ont besoin d'être nettoyées. C'est par exemple le cas de la variable "Type", pour laquelle a été collécté des chaînes de caractères qui doivent être simplifiées, ou le cas de la v ariable "com" qui contient le nom de la commune et son code INSEE. Nous procédons donc à des traitements pour optimiser la base de données collectée. 

1) Supression des bouts de chaînes de caractères précédant la valeur des variables ciblées : "Type : Appartement" -> "Appartement".
...
2) Suppression des unités de mesure pour les prix est les surfaces collectées
...
3) Conversion des nombres en format `numeric`
...
4) Ventilation de la variable "com" en deux colonnes : "nom" et "INSEE"
...
RESULT[, c("com_name ","com_insee")] <- unlist(strsplit(RESULT$com, split = " \\(" ))
